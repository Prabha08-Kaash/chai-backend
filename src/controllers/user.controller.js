...
const generateAccessAndRefreshToken = async (userId) => {
   try {
      ...
      throw new ApiError(500, "Something wemt wrong")
   }
}
...
const registerUser = asyncHandler(async (req, res) => {
   ...
   if (!createdUser) {
      throw new ApiError(500, "something went wrong while rejistering the user")
   }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
   ...
   try {
      ...
      if (!user) {
         throw new ApiError(104, "Invalid Refresh Token")
      }
      if (incomingRefreshToken !== user?.refreshToken) {
         throw new ApiError(104, "Refresh Token is expired or used")
      }
      const options = {
         httpOnly: true,
         secure: true
